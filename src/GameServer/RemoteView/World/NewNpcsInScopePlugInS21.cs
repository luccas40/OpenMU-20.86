// <copyright file="NewNpcsInScopePlugInS21.cs" company="MUnique">
...
/// </summary>
[PlugIn("NPCs in scope PlugIn S21", "The S21 implementation of the INewNpcsInScopePlugIn which is forwarding everything to the game client with specific data packets.")]
[Guid("756FF4E7-A8E4-45CE-A3AF-BD7B0F5EA056")]
[MinimumClient(21, 0, ClientLanguage.Korean)]
public class NewNpcsInScopePlugInS21 : INewNpcsInScopePlugIn
{
    private readonly RemotePlayer _player;

    /// <summary>
    /// Initializes a new instance of the <see cref="NewNpcsInScopePlugInS21"/> class.
    /// </summary>
    /// <param name="player">The player.</param>
    public NewNpcsInScopePlugInS21(RemotePlayer player) => this._player = player;
    ...
    private static async ValueTask NpcsInScopeAsync(bool isSpawned, IConnection connection, ICollection<NonPlayerCharacter> npcs)
    {
        int Write()
        {
            var size = AddNpcsToScopeS21Ref.GetRequiredSize(npcs.Count);
            var span = connection.Output.GetSpan(size)[..size];
            var packet = new AddNpcsToScopeS21Ref(span)
            {
