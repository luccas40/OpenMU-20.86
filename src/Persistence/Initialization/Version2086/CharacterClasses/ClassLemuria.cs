...
internal partial class CharacterClassInitialization
{
    ...
    protected CharacterClass CreateLemuria(CharacterClassNumber number, string name, bool isMaster, CharacterClass? nextGenerationClass, bool canGetCreated)
    {
    ...
    private CharacterClass CreateArchMage(CharacterClass nextGenerationClass)
    {
        var result = this.CreateLemuria(CharacterClassNumber.ArchMage, "Arch Mage", true, nextGenerationClass, false);
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.MasterLevel, 0, false));
        return result;
    }

    private CharacterClass CreateWoMage(CharacterClass nextGenerationClass)
    {
        return this.CreateLemuria(CharacterClassNumber.WoMage, "Wo Mage", false, nextGenerationClass, false);
    }

    private CharacterClass CreateMysticMage(CharacterClass nextGenerationClass)
    {
        return this.CreateLemuria(CharacterClassNumber.MysticMage, "Mystic Mage", true, nextGenerationClass, false);
    }

    private CharacterClass CreateBattleMage()
    {
        return this.CreateLemuria(CharacterClassNumber.BattleMage, "Battle Mage", true, null, false);
    }
}