...
internal partial class CharacterClassInitialization
{
    ...
    protected CharacterClass CreateGrowLancer(CharacterClassNumber number, string name, bool isMaster, CharacterClass? nextGenerationClass, bool canGetCreated)
    {
    ...
    private CharacterClass CreateMirageLancer(CharacterClass nextClass)
    {
        var result = this.CreateGrowLancer(CharacterClassNumber.MirageLancer, "Mirage Lancer", true, nextClass, false);
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.MasterLevel, 0, false));
        return result;
    }

    private CharacterClass CreateShiningLancer(CharacterClass nextClass)
    {
        return this.CreateGrowLancer(CharacterClassNumber.ShiningLancer, "Shining Lancer", true, nextClass, false);
    }

    private CharacterClass CreateArcaneLancer()
    {
        return this.CreateGrowLancer(CharacterClassNumber.ArcaneLancer, "Arcane Lancer", true, null, false);
    }
}