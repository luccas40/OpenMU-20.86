...
internal partial class CharacterClassInitialization
{
    ...
    private CharacterClass CreateTemplarCommander()
    {
        return this.CreateCrusader(CharacterClassNumber.TemplarCommander, "Templar Commander", true, null, false);
    }

    private CharacterClass CreateSacredPaladin(CharacterClass nextGen)
    {
        return this.CreateCrusader(CharacterClassNumber.SacredPaladin, "Sacred Paladin", true, nextGen, false);
    }

    private CharacterClass CreateMasterPaladin(CharacterClass nextGen)
    {
        var result = this.CreateCrusader(CharacterClassNumber.MasterPaladin, "Master Paladin", true, nextGen, false);
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.MasterLevel, 0, false));
        return result;
    }

    private CharacterClass CreateImpactCrusader(CharacterClass nextGen)
    {
        return this.CreateCrusader(CharacterClassNumber.ImpactCrusader, "Impact Crusader", false, nextGen, false);
    }



    /// <summary>
    /// Creates the Crusader.
    /// </summary>
    ...
    protected CharacterClass CreateCrusader(CharacterClassNumber number, string name, bool isMaster, CharacterClass? nextGenerationClass, bool canGetCreated)
    {
        ...
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.BaseVitality, 19, true));
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.BaseEnergy, 20, true));
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.CurrentHealth, 150, false));
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.CurrentMana, 40, false));
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.CurrentAbility, 1, false)); // aa
