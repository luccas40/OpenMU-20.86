...
internal partial class CharacterClassInitialization
{
    ...
    protected CharacterClass CreateGunCrusher(CharacterClassNumber number, string name, bool isMaster, CharacterClass? nextGenerationClass, bool canGetCreated)
    {
    ...
    private CharacterClass CreateMasterGunBreaker(CharacterClass nextGenerationClass)
    {
        var result = this.CreateGunCrusher(CharacterClassNumber.MasterGunBreaker, "Master Gun Breaker", true, nextGenerationClass, false);
        result.StatAttributes.Add(this.CreateStatAttributeDefinition(Stats.MasterLevel, 0, false));
        return result;
    }

    private CharacterClass CreateGunBreaker(CharacterClass nextGenerationClass)
    {
        return this.CreateGunCrusher(CharacterClassNumber.GunBreaker, "Gun Breaker", false, nextGenerationClass, false);
    }

    private CharacterClass CreateHeistGunCrusher(CharacterClass nextGenerationClass)
    {
        return this.CreateGunCrusher(CharacterClassNumber.HeistGunCrusher, "Heist Gun Crusher", true, nextGenerationClass, false);
    }

    private CharacterClass CreateMagnusGunCrusher()
    {
        return this.CreateGunCrusher(CharacterClassNumber.MagnusGunCrusher, "Magnus Gun Crusher", true, null, false);
    }
}