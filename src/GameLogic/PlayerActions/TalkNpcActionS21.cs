// <copyright file="TalkNpcActionS21.cs" company="MUnique">
...
public class TalkNpcActionS21
{
    ...
    public async ValueTask TalkToNpcAsync(Player player, NonPlayerCharacter npc)
    {
        ...
        if (npcStats.MerchantStore != null && npcStats.MerchantStore.Items.Count > 0)
        {
            ...
            await player.InvokeViewPlugInAsync<IOpenNpcWindowPlugIn>(p => p.OpenNpcWindowAsync(npcStats.NpcWindow != NpcWindow.Undefined ? npcStats.NpcWindow : NpcWindow.Merchant, 1)).ConfigureAwait(false);
